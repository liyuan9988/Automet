...
class DataHandler:
    ...
    def get_negative_CPA(self):
        id_hashed = []
        target_date = []
        timepoint = []
        duration = np.empty(self.target_table.shape[0], dtype=int)
        for idx in range(self.target_table.shape[0]):
            start = self.target_table["enter_Einstein"].iloc[idx]
            end = self.target_table["discharge_Einstein"].iloc[idx]
            duration = (end - start).days + 1
            id_hashed_candi = self.target_table["id_hashed"].iloc[idx]
            for diff in range(duration):
                date = start + pd.to_timedelta("%ddays" % diff)
                for timepoint_candi in range(self.config["learning_param"]["n_target_timepoints_per_day"]):

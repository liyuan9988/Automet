...
def train_one(ids):
    ...
    for i in range(n_model):
        print("train %d"%i)
        X,y = build_dataset(train_path, train_target_table, pos_idx_train ,vital = True, labo = True,    n_timepoint=n_timepoint, nDay=n_day)
        print(X.shape)
    ...
    X,y_test = build_dataset(test_path, test_target_table,pos_idx_test, vital = True, labo = True , n_negative=10000,  n_timepoint=n_timepoint,nDay=n_day)
